...
class BlUtil:
    class Object:
        ...
        def vertex_location(obj, i, undeformed=False):
            ...
            if obj.type == 'MESH':
                ...
                if (i >= len(obj.data.points)): return Vector()
                p = obj.data.points[i]
                return Vector(p.co if undeformed else p.co_deform)
            else:
                return Vector()
    ...
    class Scene:
        ...
        def ray_cast(scene, subset, origin, direction, distance=1.70141e+38):
            # returns (result, location, normal, index, object, matrix)
            if isinstance(subset, bpy.types.ViewLayer): subset = subset.depsgraph
    ...
    class Orientation:
        ...
        def create(context, name, matrix=None, overwrite=False, normalize=True):
            ...
            with bpy.context.temp_override(**attrs_to_dict(context)):
                bpy.ops.transform.create_orientation(name=name, use_view=True, use=False, overwrite=overwrite)
...
class BpyPath:
    ...
    def relpath(path, *, start=None):
        try:
            ...
            return bpy.path.relpath(path, start=start)
        except ValueError:
            return path
